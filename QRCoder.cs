...
namespace QRCoder
{
	public class QRCodeGenerator
	{
		...
		public QRCode CreateQrCode(string plainText, ECCLevel eccLevel, bool utf8BOM = false)
		{
			...
			List<CGRect> blockedModules = new List<CGRect>();
		...
		private static class ModulePlacer
		{
			...
			public static void PlaceFormat(ref QRCode qrCode, string formatStr)
			{
				...
				for (int i = 0; i < 15; i++)
				{
					var p1 = new CGPoint(modules[i, 0], modules[i, 1]);
					var p2 = new CGPoint(modules[i, 2], modules[i, 3]);

					qrCode.ModuleMatrix[Convert.ToInt32(p1.Y)][Convert.ToInt32(p1.X)] = fStr[i] == '1' ? true : false;
					qrCode.ModuleMatrix[Convert.ToInt32(p2.Y)][Convert.ToInt32(p2.X)] = fStr[i] == '1' ? true : false;
				}
			}

			public static int MaskCode(ref QRCode qrCode, int version, ref List<CGRect> blockedModules)
			{
				...
				foreach (var pattern in typeof(MaskPattern).GetMethods())
				{
					if (pattern.Name.Length == 8 && pattern.Name.Substring(0, 7) == "Pattern")
					{
						...
						for (int x = 0; x < size; x++)
						{
							for (int y = 0; y < size; y++)
							{
								if (!IsBlocked(new CGRect(x, y, 1, 1), blockedModules))
								{
			...
			public static void PlaceDataWords(ref QRCode qrCode, string data, ref List<CGRect> blockedModules)
			{
				...
				for (int x = size - 1; x >= 0; x = x - 2)
				{
					...
					for (int yMod = 1; yMod <= size; yMod++)
					{
						...
						{
							y = size - yMod;
							if (datawords.Count > 0 && !IsBlocked(new CGRect(x, y, 1, 1), blockedModules))
								qrCode.ModuleMatrix[y][x] = datawords.Dequeue();
							if (datawords.Count > 0 && x > 0 && !IsBlocked(new CGRect(x - 1, y, 1, 1), blockedModules))
			...
			public static void ReserveSeperatorAreas(int size, ref List<CGRect> blockedModules)
			{
				blockedModules.AddRange(new CGRect[]{
					new CGRect(7, 0, 1, 8),
					new CGRect(0, 7, 7, 1),
					new CGRect(0, size-8, 8, 1),
					new CGRect(7, size-7, 1, 7),
					new CGRect(size-8, 0, 1, 8),
					new CGRect(size-7, 7, 7, 1)
				});
			}

			public static void ReserveVersionAreas(int size, int version, ref List<CGRect> blockedModules)
			{
				blockedModules.AddRange(new CGRect[]{
					new CGRect(8, 0, 1, 6),
					new CGRect(8, 7, 1, 1),
					new CGRect(0, 8, 6, 1),
					new CGRect(7, 8, 2, 1),
					new CGRect(size-8, 8, 8, 1),
					new CGRect(8, size-7, 1, 7)
				});

				if (version >= 7)
				{
					blockedModules.AddRange(new CGRect[]{
						new CGRect(size-11, 0, 3, 6),
						new CGRect(0, size-11, 6, 3)
					});
				}
			}
			public static void PlaceDarkModule(ref QRCode qrCode, int version, ref List<CGRect> blockedModules)
			{
				qrCode.ModuleMatrix[4 * version + 9][8] = true;

				blockedModules.Add(new CGRect(8, 4 * version + 9, 1, 1));
			}

			public static void PlaceFinderPatterns(ref QRCode qrCode, ref List<CGRect> blockedModules)
			{
				var size = qrCode.ModuleMatrix.Count;
				...
				for (int i = 0; i < 6; i = i + 2)
				{
					...
					blockedModules.Add(new CGRect(locations[i], locations[i + 1], 7, 7));
				}
			}

			public static void PlaceAlignmentPatterns(ref QRCode qrCode, List<CGPoint> alignmentPatternLocations, ref List<CGRect> blockedModules)
			{
				foreach (var loc in alignmentPatternLocations)
				{
					var alignmentPatternRect = new CGRect(loc.X, loc.Y, 5, 5);
					var blocked = false;
					...
					for (int x = 0; x < 5; x++)
					{
						for (int y = 0; y < 5; y++)
						{
							...
							{
								qrCode.ModuleMatrix[Convert.ToInt32(loc.Y + y)][Convert.ToInt32(loc.X + x)] = true;
							}
						}
					}
					blockedModules.Add(new CGRect(loc.X, loc.Y, 5, 5));
				}
			}

			public static void PlaceTimingPatterns(ref QRCode qrCode, ref List<CGRect> blockedModules)
			{
				var size = qrCode.ModuleMatrix.Count;
				...
				blockedModules.AddRange(new CGRect[]{
					new CGRect(6, 8, 1, size-16),
					new CGRect(8, 6, size-16, 1)
				});
			}

			private static bool Intersects(CGRect r1, CGRect r2)
			{
				return r2.X < r1.X + r1.Width && r1.X < r2.X + r2.Width && r2.Y < r1.Y + r1.Height && r1.Y < r2.Y + r2.Height;
			}

			private static bool IsBlocked(CGRect r1, List<CGRect> blockedModules)
			{
				var isBlocked = false;
				...
				for (int x = 0; x < 7; x++)
				{
					if (alignmentPatternBaseValues[i + x] != 0)
					{
						for (int y = 0; y < 7; y++)
						{
							...
							{
								var p = new CGPoint(alignmentPatternBaseValues[i + x] - 2, alignmentPatternBaseValues[i + y] - 2);
		...
		{
			public int Version;
			public List<CGPoint> PatternPositions;
		}
		...
		public class QRCode
		{
			...
			public NSImage GetGraphic(int pixelsPerModule)
			{
				var dark = NSColor.Black.CGColor;
				var white = NSColor.White.CGColor;
				return GetGraphic(pixelsPerModule,dark , white);
			}



			public NSImage GetGraphic(int pixelsPerModule, CGColor darkColor, CGColor lightColor)
			{
				var size = ModuleMatrix.Count * pixelsPerModule;



				var cs = CGColorSpace.CreateWithName(CGColorSpaceNames.GenericRGB);
				var bytePerRow = size * 4;
				var byteCount = bytePerRow * size;

				var data = new byte[byteCount];
				var context = new CGBitmapContext(data, size, size, 8, bytePerRow, cs, CGImageAlphaInfo.PremultipliedLast);
				...
				for (int x = 0; x < size; x = x + pixelsPerModule)
				{
					for (int y = 0; y < size; y = y + pixelsPerModule)
					{
						...
						if (module) {
							...
							context.FillRect (new CGRect (x, size-y, pixelsPerModule, pixelsPerModule));
				...
				var imag = new NSImage (cgImage,new CGSize (size, size));
				context.Dispose ();
				return imag;
			}
